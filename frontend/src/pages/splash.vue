...
onMounted(async () => {
  ...
  // After "Ready to launch" shows for 1 second, transition to white page
  await new Promise((resolve) => setTimeout(resolve, 1000));

  showWhitePage.value = true;

  // After white page appears, show "Ready to launch" message
...
<template>
  <div
    v-if="showWhitePage"
    class="white-page min-h-screen w-full bg-white flex flex-col items-center justify-center"
  >
    <transition name="fade">
      <div
        v-if="showReadyMessage"
        class="text-black text-2xl font-bold flex items-center"
      >
        <CheckCircle2 class="w-6 h-6 mr-2 text-green-600" />
        Ready to launch
      </div>
    </transition>
  </div>

  <!-- Original splash screen that disappears after loading -->
  <div
    v-else
  ...
  >
    ...
    <div class="relative mb-12">
      ...
      <div class="relative z-10 flex items-center justify-center">
        <div class="logo-container relative">
          ...
          >
            <Shield class="h-14 w-14 text-white" />
          </div>
    ...
    <div class="absolute bottom-4 flex flex-col items-center justify-center">
      <p class="text-xs text-gray-400 mb-1">Version {{ currentVersion }}</p>
      <p class="text-xs text-gray-500">&copy; 2025 Pitjarus Teknologi</p>
    </div>
  </div>
</template>
