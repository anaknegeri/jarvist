...
const onSubmit = form.handleSubmit(async (values) => {
  ...
  try {
    const { success, message, error } = await createCamera(value);
    if (success) {
      toast.toast({
        title: "Success",
      ...
      toast.toast({
        title: "Error",
        description: message || error || "Failed to add camera",
        variant: "destructive",
      });
    }
  } catch (error) {
...
async function handleAddLocation(value: { name: string; description: string }) {
  try {
    const { success, data, message, error } = await createLocation(value);
    if (success) {
      await listLocations();
      ...
      toast.toast({
        title: "Error",
        description: message || error || "Failed to add location",
        variant: "destructive",
      });
    }
  } catch (error) {
  ...
  <div class="flex flex-col h-full">
    ...
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center gap-3">
        ...
        <h2 class="text-lg font-medium text-gray-800">Add New Camera</h2>
      </div>
